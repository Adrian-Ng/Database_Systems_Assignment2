	...
	WITH cteRandomPairs AS (
		-- EVALUATE EVERY POSSIBLE COMBINATION OF SONG/ARTIST PAIRS
		SELECT
			SongID
		,	ArtistID
		FROM		mus.Song AS s
	...
	,	cteMainArtist AS (
		--FILTER SO THAT WE PICK THE FIRST RANDOM SET OF SONGS 
		SELECT
			SongID
		,	ArtistID
		FROM		ctePartition
	...
	--PLAYLIST TRACK
	WITH cteRandomPairs AS (
		-- EVALUATE EVERY POSSIBLE COMBINATION OF SONG/PLAYLIST PAIRS
		SELECT
			PlaylistID
		,	SongID
